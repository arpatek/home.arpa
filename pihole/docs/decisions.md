...
## netrunner's address is static, not DHCP

**Decision.** `netrunner` holds `10.33.111.141` through a NetworkManager keyfile (`eth0-static.nmconnection`, `0600 root:root`), never through a lease.

**Why.**
A host that serves DHCP cannot depend on DHCP for its own address. The dependency is circular and self-locking: NetworkManager falls back to a DHCP-only connection, the segment's only DHCP server is Pi-hole, Pi-hole runs on `netrunner`, and `netrunner` has no address — so it waits indefinitely for a lease that only it could issue. This is not theoretical; it took the host down for roughly 75 minutes on 2026-09-08 after a power cut, and it could not self-recover through any number of reboots. `may-fail=false` is deliberate: `NetworkManager-wait-online` blocks until eth0 actually holds the address, trading a slower boot for the guarantee that no service starts before its bind address exists. The same reasoning that rejected a Pi-hole VM for having a circular dependency on the infrastructure it serves applies here — the address is infrastructure the host must own outright. See [gotchas.md](gotchas.md) for the failure signature.

That outage also validated the single-homed DHCP trade above: the gap was ~75 minutes against a 24h lease time, so existing clients kept both resolvers and continued resolving via `edgerunner` throughout.

## Persistent journald over the Raspberry Pi OS volatile default

**Decision.** Both Pis override the vendor `Storage=volatile` with `Storage=persistent`, capped at `SystemMaxUse=200M` and `MaxRetentionSec=1month`.

**Why.**
Raspberry Pi OS ships `Storage=volatile` to spare the SD card, which is defensible in the general case and wrong for load-bearing infrastructure. The cost is paid exactly when it hurts most: an unclean shutdown destroys every log describing why the host went down. The 2026-09-08 outage had to be reconstructed from `cloud-init.log` and file mtimes because the failure left no trace at all, turning a config bug into an evening of forensics. The wear argument does not survive contact with what these hosts already do — `pihole-FTL`'s SQLite database and WAL write continuously to the same card, so a capped 200M journal is marginal additional wear. Caps plus `Compress=yes` bound the exposure; `SyncIntervalSec=5m` keeps writes batched rather than per-message. Neither Pi runs rsyslog, so journald is the only log store there is.
