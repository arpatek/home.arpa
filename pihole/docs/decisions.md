...
**Why.**
A host that serves DHCP cannot depend on DHCP for its own address. The dependency is circular and self-locking: NetworkManager falls back to a DHCP-only connection, the segment's only DHCP server is Pi-hole, Pi-hole runs on `netrunner`, and `netrunner` has no address — so it waits indefinitely for a lease that only it could issue. This is not theoretical; it took the host down for five hours on 2026-09-08 after a power cut, and it could not self-recover through any number of reboots. `may-fail=false` is deliberate: `NetworkManager-wait-online` blocks until eth0 actually holds the address, trading a slower boot for the guarantee that no service starts before its bind address exists. The same reasoning that rejected a Pi-hole VM for having a circular dependency on the infrastructure it serves applies here — the address is infrastructure the host must own outright. See [gotchas.md](gotchas.md) for the failure signature.

That outage also validated the single-homed DHCP trade above: the gap was ~5h against a 24h lease time, so existing clients kept both resolvers and continued resolving via `edgerunner` throughout.
