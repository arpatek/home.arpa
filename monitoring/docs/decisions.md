...
**Decision.** One central host (`netwatch`) runs Prometheus, Loki, and Grafana.
...
A snapshot of `/opt/monitoring/` on `netwatch` captures everything.
...
The decision to revisit would be: at the point where `netwatch` is consistently at 80%+ memory or query latency exceeds a few seconds, federation becomes worth the complexity.
...
**Decision.** On hosts that already run Docker (`netwatch`, `soulkiller`), the monitoring agents — node_exporter, cAdvisor, Alloy — run as containers, defined in a docker-compose.yml.
On `mikoshi` (Rocky Linux, no Docker), node_exporter and Alloy run as native systemd services with the binaries installed under `/usr/local/bin`.
...
On `mikoshi` specifically, Docker would compete with FreeIPA for resources and add an attack surface to a host that's deliberately kept minimal.
...
A future-me debugging on `mikoshi` doesn't have to context-switch between "is this a Docker thing or a systemd thing?"
...
Installing Docker on `mikoshi` would mean either using the upstream `docker-ce` repo (going off the Red Hat path) or using Podman with `podman-compose` (a different tool that's not 1:1 compatible with Docker Compose).
...
If `mikoshi` ever grows to need containers — say, for a tool that only ships as an OCI image — the right runtime on RHEL is Podman, not Docker.
...
**Separate `/opt/monitoring-agents/` compose project on non-server hosts.** On `soulkiller`, the monitoring agents live in their own compose project rather than being added to `/opt/gitea/docker-compose.yml`. The reasoning is that monitoring is a secondary function on that host — the host's job is running Gitea. Restarting Gitea (for an upgrade, a config change, anything) shouldn't bounce the monitoring agents. Two separate compose projects mean each is operated independently. The `-agents` suffix in the directory name distinguishes "this host runs monitoring agents" from "this host runs the monitoring stack."
...
**Short `host` label on logs (`netwatch`, not the FQDN).** Loki labels carry a `host` value that identifies which machine the logs came from. I used the short hostname rather than the FQDN. The reasoning is that the FQDN domain (`home.arpa`) is the same for every host in this stack, so it adds noise to every log query without disambiguating anything. If this stack ever spans multiple domains, the label gets reconsidered.
